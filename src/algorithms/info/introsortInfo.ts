...
export const introsortInfo: AlgorithmInfo = {
  name: 'Intro Sort',
  description: 'Introspective Sort (Introsort) is a sophisticated hybrid sorting algorithm that combines the best characteristics of Quick Sort, Heap Sort, and Insertion Sort to achieve both excellent average performance and guaranteed worst-case behavior. The algorithm begins with Quick Sort, leveraging its exceptional average-case performance and cache efficiency. However, it monitors the recursion depth, and when it exceeds a threshold (typically 2×log n), it switches to Heap Sort to avoid Quick Sort\'s quadratic worst-case scenario. For small subarrays (usually fewer than 16 elements), it employs Insertion Sort, which excels on small datasets. This intelligent combination makes Introsort the algorithm of choice for many standard library implementations, including C++\'s std::sort, as it provides the speed of Quick Sort with the reliability of Heap Sort and the efficiency of Insertion Sort for small inputs.',
  complexity: {
    time: { 
      best: 'O(n log n)', 
      average: 'O(n log n)', 
