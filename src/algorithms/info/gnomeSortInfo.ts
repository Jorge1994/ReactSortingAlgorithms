...
export const gnomeSortInfo: AlgorithmInfo = {
  name: "Gnome Sort",
  description: "Gnome Sort, also known as Stupid Sort, is a delightfully simple sorting algorithm that mimics the way a garden gnome might sort a line of flower pots. Similar to Insertion Sort in its approach of building a sorted portion one element at a time, Gnome Sort uses a different strategy: it moves forward through the array, and whenever it encounters two adjacent elements that are out of order, it swaps them and steps backward to check if the newly placed element needs to be moved further back. This process continues until the element finds its correct position, at which point the algorithm resumes moving forward. While Gnome Sort has the same time complexity as Bubble Sort, its straightforward logic makes it one of the easiest sorting algorithms to understand and implement. The algorithm is adaptive, performing significantly better on partially sorted data, and maintains stability throughout the sorting process.",
  stable: true,
  inPlace: true,
  online: false,
